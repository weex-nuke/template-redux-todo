...
import { View, Text, ListView, Checkbox, Transition, Cell, Touchable } from 'nuke';
    ...
    return (
      <ListView _autoWrapCell={false} style={[styles.listContainer, style]}>
        {dataSource.toDoList.map(item => (
          <Cell key={`cell_${item.id}`} id={`cell_${item.id}`} style={{ alignItems: 'center' }}>
            <View id={`cell_item_${item.id}`} style={styles.cellItem} className="nuke-border-fix-1">
              <Checkbox
                key={`cell_checkbox_${item.id}`}
                defaultChecked={false}
                size="small"
                style={styles.checkbox}
                onChange={this.changeItemStatus.bind(this, item)}
              />
        ...
        {this.state.showDone &&
          dataSource.doneList.map(item => (
            <Cell key={`cell_${item.id}`} id={`cell_${item.id}`} style={{ alignItems: 'center' }}>
              <Touchable
                id={`cell_item_${item.id}`}
                style={[styles.cellItem, styles.done]}
                className="nuke-border-fix-1"
              >
                <Checkbox
                  key={`cell_checkbox_${item.id}`}
                  checked
                  size="small"
                  style={styles.checkbox}
                  checkedStyle={styles.checkbox_checked}
                />
